...
function selectServer(idx, sources) {
  ...
  if (frame) {
    ...
    var db=document.getElementById('dmBlocker');if(url.indexOf('dailymotion.com')>=0){if(!/ui_endscreen=/.test(url))url+=(url.indexOf('?')>=0?'&':'?')+'ui_endscreen=0';if(db)db.classList.add('is-active')}else{if(db)db.classList.remove('is-active')}
    frame.src = url;
  }
